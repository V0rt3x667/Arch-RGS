...
archrgs_module_desc="Sega Dreamcast Emulator"
...
function configure_rgs-em-reicast() {
    ...
    addEmulator 1 "$md_id" "dreamcast" "$md_inst/bin/reicast.sh pulse ${params[*]}"
    ...
    install -Dm755 "$md_inst"/share/reicast/mappings/*.cfg "$md_conf_root"/dreamcast/mappings/

    chown -R "$user":"$user" "$md_conf_root/dreamcast"

    #Copy hotkey remapping start script
    install -Dm755 "$md_data/reicast.sh" "$md_inst/bin/"
