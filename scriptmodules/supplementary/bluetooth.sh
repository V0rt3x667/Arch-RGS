...
archrgs_module_id="bluetooth"
archrgs_module_desc="Configure Bluetooth Devices"
...
function depends_bluetooth() {
    local depends=(bluez bluez-tools bluez-lib python-dbus python-gobject)
    getDepends "${depends[@]}"
}
...
[Service]
Type=$type
ExecStart=/bin/bash "$scriptdir/archrgs-packages.sh" bluetooth boot
