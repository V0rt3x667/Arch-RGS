...
function _update_hook_archrgs_menu() {
    # To show as installed when upgrading to archrgs-setup.
    if ! archrgs_isInstalled "$md_idx" && [[ -f "$home/.emulationstation/gamelists/archrgs/gamelist.xml" ]]; then
        mkdir -p "$md_inst"
        # To stop older scripts removing when launching from the archrgs menu in ES due to not using exec or exiting after running archrgs-setup from this module.
