...
archrgs_module_id="retronetplay"
archrgs_module_desc="RetroNetplay"
archrgs_module_section="config"

function archrgs_retronet_saveconfig() {
...
function archrgs_retronet_loadconfig() {
  if [[ -f "$configdir/all/retronetplay.cfg" ]]; then
    ...
    __netplaynickname="Arch-RGS"
  fi
}

function archrgs_retronet_mode() {
...
function archrgs_retronet_port() {
...
function archrgs_retronet_hostip() {
...
function archrgs_retronet_nickname() {
  cmd=(dialog --backtitle "$__backtitle" --inputbox "Please enter the nickname you wish to use (default: Arch-RGS)" 22 76 $__netplaynickname)
  choice=$("${cmd[@]}" 2>&1 >/dev/tty)
  if [[ -n "$choice" ]]; then
...
function gui_retronetplay() {
  archrgs_retronet_loadconfig

  local ip_int=$(ip addr | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | grep -v '127.0.0.1')
  ...
  while true; do
    ...
    if [[ -n "$choice" ]]; then
      case "$choice" in
      1)
        archrgs_retronet_mode
        ;;
      2)
        archrgs_retronet_port
        ;;
      3)
        archrgs_retronet_hostip
        ;;
      4)
        archrgs_retronet_nickname
        ;;
      5)
        archrgs_retronet_saveconfig
        ;;
      esac
    else
      break
