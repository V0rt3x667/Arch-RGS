...
archrgs_module_id="bashwelcometweak"
archrgs_module_desc="Bash Welcome Tweak (shows additional system info on login)"
    ...
    for i in "${!logo[@]}"; do
        ...
        case "$i" in
            ...
            9)
                ...
                ;;
            10)
                out+="${fgwht}The Arch-RGS Project."
                ;;
        esac
        out+="\n"
    done
...
function remove_bashwelcometweak() {
    sed -i '/ARCH-RGS PROFILE START/,/ARCH-RGS PROFILE END/d' "$home/.bashrc"
}
...
function gui_bashwelcometweak() {
    ...
    if [[ -n "$choice" ]]; then
        case "$choice" in
            1)
                archrgs_callModule bashwelcometweak install
                printMsgs "dialog" "Installed Bash Welcome Tweak."
                ;;
            2)
                archrgs_callModule bashwelcometweak remove
