...
function onstart_rgs-fe-emulationstation_joystick() {
  ...
  if [[ $(xmlstarlet sel -t -v "count(/inputList/inputConfig[@deviceGUID=\"$DEVICE_GUID\"])" "$es_conf") -eq 0 ]]; then
    ...
    if [[ $(xmlstarlet sel -t -v "count(/inputList/inputConfig[@deviceName=\"$DEVICE_NAME\"][not(@deviceGUID)])" "$es_conf") -eq 0 ]]; then
      ...
      xmlstarlet ed -L -s "/inputList" -t elem -n newInputConfig -v "" \
        ...
        -r //newInputConfig -v inputConfig "$es_conf"
    else
      ##ADD deviceGUID TO inputConfig
      xmlstarlet ed -L \
        -i "/inputList/inputConfig[@deviceName=\"$DEVICE_NAME\"]" -t attr -n "deviceGUID" -v "$DEVICE_GUID" "$es_conf"
    fi
  fi
}

function map_rgs-fe-emulationstation_joystick() {
  local input_name="$1"
  local input_type="$2"
  local input_id="$3"
  local input_value="$4"
  ...
  *)
  ...
  ##IF DEVICE ALREADY EXISTS, UPDATE IT
    ...
    xmlstarlet ed -L \
      ...
      -u "/inputList/inputConfig[@deviceGUID=\"$DEVICE_GUID\"]/input[@name=\"$key\"]/@value" -v "$input_value" "$es_conf"
  fi
}

function onstart_rgs-fe-emulationstation_keyboard() {
  onstart_rgs-fe-emulationstation_joystick "$@"
}

function map_rgs-fe-emulationstation_keyboard() {
  map_rgs-fe-emulationstation_joystick "$@"
}

function onstart_rgs-fe-emulationstation_cec() {
  onstart_rgs-fe-emulationstation_joystick "$@"
}
